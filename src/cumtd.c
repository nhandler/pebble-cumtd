...
static char *stops[NUM_MENU_ITEMS];
static char *codes[NUM_MENU_ITEMS];
static int menu_size = 0;

static char *headsigns[DEP_NUM_MENU_ITEMS];
static char *times[DEP_NUM_MENU_ITEMS];
...
static void append_menu(char *stop, char *code) {
  ...
  stops[menu_size] = malloc(sizeof(char)*(strlen(stop)+1));
  strcpy(stops[menu_size], stop);
  codes[menu_size] = malloc(sizeof(char)*(strlen(code)+1));
  strcpy(codes[menu_size], code);
  menu_size += 1;
...
static void dep_append_menu(char *headsign, char *minutes) {
  ...
  headsigns[dep_menu_size] = malloc(sizeof(char)*(strlen(headsign)+1));
  strcpy(headsigns[dep_menu_size], headsign);
  times[dep_menu_size] = malloc(sizeof(char)*(strlen(minutes)+1));
  strcpy(times[dep_menu_size], minutes);
  dep_menu_size += 1;
...
static void menu_draw_row_callback(GContext *ctx, const Layer *cell_layer, MenuIndex *cell_index, void *data) {
  if (cell_index->section == 0) {
    APP_LOG(APP_LOG_LEVEL_DEBUG, "Drawing Row: %s, %s", stops[cell_index->row], codes[cell_index->row]);
    menu_cell_basic_draw(ctx, cell_layer, stops[cell_index->row], codes[cell_index->row], NULL);
  }
}

static void dep_menu_draw_row_callback(GContext *ctx, const Layer *cell_layer, MenuIndex *cell_index, void *data) {
  if (cell_index->section == 0) {
    APP_LOG(APP_LOG_LEVEL_DEBUG, "Drawing Row: %s, %s", headsigns[cell_index->row], times[cell_index->row]);
    menu_cell_basic_draw(ctx, cell_layer, headsigns[cell_index->row], times[cell_index->row], NULL);
  }
}
...
void menu_select_callback(MenuLayer *menu_layer, MenuIndex *cell_index, void *data) {
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "Sending Subtitle: %s", codes[cell_index->row]);
...
/*  if (cell_index->section == 0) {
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "Sending Subtitle: %s", times[cell_index->row]);
}
...
static void dep_window_unload(Window *window) {
  int i;
  for (i = 0 ; i < DEP_NUM_MENU_ITEMS ; i += 1) {
    free(headsigns[i]);
    free(times[i]);
  }
  dep_menu_size = 0;
